...
import {
  Animated,
  Easing, Dimensions, TouchableWithoutFeedback, TouchableOpacity
} from 'react-native';
...
const Bubble: () => Node = ({delayBubble}) => {
  ...
  const startAnimate = () => {
    ...
    for (let i = randY; i > -4; i--) {
      m.push(Animated.timing(valueXY, {
        toValue:
          {
            x: amply * Math.sin((Dimensions.get('window').width / n * i) / 50) + x,
            y: (Dimensions.get('window').height + 100) / n * i
          },
  ...
  return (
    <Animated.View style={{
      transform: [{translateY: valueXY.y}, {translateX: valueXY.x}],
      position: "absolute",
    }}>
